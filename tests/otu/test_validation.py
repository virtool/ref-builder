...
from ref_builder.otu.validate import check_otu_for_validity, check_repo_for_invalid_otus
...
class TestValidateOTU:
    ...
    def test_ok(self, scratch_repo: Repo):
        ...
        for otu_metadata in scratch_repo.iter_minimal_otus():
            assert check_otu_for_validity(
                scratch_repo.get_otu_by_taxid(otu_metadata.taxid)
            )
    ...
    def test_no_isolates(self, scratch_repo: Repo):
        ...
        assert check_otu_for_validity(otu_clean)
        ...
        assert not check_otu_for_validity(otu_invalid)
